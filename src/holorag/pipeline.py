...
class HoloRAG:
    def __init__(self, config: Optional[HoloRAGConfig] = None) -> None:
        ...
        self.retriever = Retriever(self.config, self.embedder, self.llm_client)
        self.page_rank = GranularityPageRank(self.config)
        self.reader = QAReader(self.config, self.llm_client)
        self.state: Optional[Dict] = None

    def index(self, documents: List[Dict[str, str]]) -> Dict:
        self.llm_client.reset_stats()
        logger.info("Building HoloRAG index for %d documents", len(documents))
