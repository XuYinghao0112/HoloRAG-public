...
def load_samples(dataset_file: str, dataset_format: str) -> List[Dict[str, Any]]:
    ...
    if not isinstance(payload, list):
        raise ValueError(f"Expected top-level list in dataset: {dataset_file}")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dataset_format", type=str, default="auto", choices=["auto", "musique_jsonl", "2wiki_json", "canonical_json"])
...
def main() -> None:
    ...
    dataset_tag = "2wiki" if dataset_format == "2wiki_json" else ("musique" if dataset_format == "musique_jsonl" else "dataset")
    run_name = args.run_name.strip() or f"{dataset_tag}_{args.num_eval_queries}_seed{args.seed}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
